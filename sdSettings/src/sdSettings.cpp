...
sdSettings::sdSettings(String fileLocation, int chipSelect, int maxLines)
{
    ...
    settingName = new String[_maxLines];
    settingValue = new String[_maxLines];
...
void sdSettings::packAndSave(bool debugMode = true)
{
    ...
    if (fileExists())
    {
        if (myData)
        {
            ...
            {
                String fullLine = String(settingName[a]) + String(": ") + String(settingValue[a]);
